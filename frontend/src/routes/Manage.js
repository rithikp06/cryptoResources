...
const Manage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const temp = resources.map((resource) => {
      return resource.url;
    });
    console.log("temp", temp);
    const idx = temp.findIndex((resource) => {
      return resource == url;
    });
    ...
    <div style={{ margin: 40 }}>
      ...
      <Form style={{ maxWidth: 700 }} onSubmit={handleSubmit}>
        <Form.Group className="mb-3" controlId="formBasicEmail">
          ...
          />
          {submitted && name.length == 0 && (
            <Form.Text className="text-muted">Name cannot be empty</Form.Text>
          )}
        </Form.Group>
        ...
        <Form.Group className="mb-3" controlId="formBasicPassword">
          ...
          />
          {submitted && url.length == 0 && (
            <Form.Text className="text-muted">URL cannot be empty</Form.Text>
          )}
        </Form.Group>
        <Form.Group className="mb-3" controlId="formBasicPassword">
          ...
          {submitted && level.length == 0 && (
            <Form.Text className="text-muted">Select a level</Form.Text>
          )}
        </Form.Group>
        <Form.Group className="mb-3" controlId="formBasicPassword">
          ...
          />
          {submitted && topic.length == 0 && (
            <Form.Text className="text-muted">Topic cannot be empty</Form.Text>
          )}
        </Form.Group>
      ...
      <h4 style={{ marginTop: 30, marginBottom: -10 }}>Delete Resources</h4>
      <ManageResources
        method="DELETE"
        reverseDelete={true}
        key={update}
        url={"http://localhost:3001/resources/update"}
        favs={resources.map((resource) => {
          return resource.url;
        })}
        resources={resources}
      />
