...
test("Signup test", () => {
  ...
  fireEvent.change(email, {
    target: { value: "adjfklajdkfljaklfjkladsdjfasf@gmail.com" },
  });
...
//test 3
test("Signout test", () => {
  const rendered = render(<NavigationBar />);
...
test("Invalid Crypto Price", () => {
  const rendered = render(<CryptoPrice token="fake" />);
...
test("Valid Crypto Price", () => {
  const rendered = render(<CryptoPrice token="ethereum" />);
  const price = rendered.getByTestId("price");
  setTimeout(() => {
